...
describe('FetchStudentsListHome function', () => {
  ...
  it('should fetch student list for home', async () => {
    ...
    const expectedUrl = 'https://itaperfils.eurecatacademy.org/api/v1/student/list/for-home?specialization=role1,role2&tags=tag1,tag2';

    const mockData = [{ id: 1, name: 'Student 1' }, { id: 2, name: 'Student 2' }];

    mockAxios.onGet(expectedUrl).reply(200, mockData);

    const result = await FetchStudentsListHome(selectedRoles, selectedTags);
  ...
  it('should handle errors', async () => {
    ...
    await expect(FetchStudentsListHome(selectedRoles, selectedTags)).rejects.toThrow();
  });
});
